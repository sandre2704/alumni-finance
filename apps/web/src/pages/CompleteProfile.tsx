...
export const CompleteProfile = () => {
    ...
    const checkUsername = async (value: string) => {
        ...
        try {
            const response = await fetch(`${API_URL}/api/profile/check-username/${value}`, {
                credentials: 'include',
            });
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            await logout();
            navigate('/login', {
                state: { message: 'Registrasi berhasil! Silakan login dengan username atau email Anda.' }
            });
        } catch (err: any) {
        ...
        <div className="bg-background-light dark:bg-background-dark font-display text-slate-900 dark:text-white min-h-screen flex flex-col items-center justify-center p-4 relative">
            ...
            <div className="relative z-10 w-full max-w-[480px] bg-white dark:bg-surface-dark rounded-xl shadow-2xl border border-slate-200 dark:border-border-dark overflow-hidden">
                <div className="px-8 pt-10 pb-2 text-center">
                    ...
                    <h1 className="text-3xl font-bold leading-tight tracking-tight mb-2">Lengkapi Profil</h1>
                    <p className="text-slate-500 dark:text-[#9292c8] text-base font-normal">
                        Halo {user?.name || 'User'}! Buat username dan password untuk login.
                    </p>
                </div>
                    ...
                    <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
                        ...
                        <label className="flex flex-col w-full">
                            ...
                            <div className="relative flex items-center">
                                ...
                                />
                                {isCheckingUsername && (
                                    <span className="absolute right-4 text-slate-400 text-sm">Checking...</span>
                                )}
                            </div>
                            {usernameError && (
                                <span className="text-xs text-red-500 mt-1">{usernameError}</span>
                            )}
                            <span className="text-xs text-slate-400 mt-1">3-20 karakter, huruf, angka, underscore</span>
                        </label>

                        <label className="flex flex-col w-full">
                            <span className="text-sm font-medium leading-normal pb-2 text-slate-900 dark:text-white">Password</span>
                            <div className="relative flex items-center">
                                <span className="absolute left-4 material-symbols-outlined text-[#9292c8]">lock</span>
                                <input
                                    value={password}
                        ...
                        <label className="flex flex-col w-full">
                            <span className="text-sm font-medium leading-normal pb-2 text-slate-900 dark:text-white">Konfirmasi Password</span>
                            <div className="relative flex items-center">
                                <span className="absolute left-4 material-symbols-outlined text-[#9292c8]">lock</span>
                                <input
