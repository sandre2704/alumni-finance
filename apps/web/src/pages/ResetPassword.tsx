...
export const ResetPassword = () => {
    ...
    // Check for 'token' or 'code' - better-auth usually sends 'token' or expects verification via link click which might pass code
    // Actually better-auth's reset password link usually sends a token.
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        if (!token) {
            setError('Link reset password tidak valid (token hilang). Silakan request ulang.');
            return;
        }
        ...
        try {
            ...
            setError(err?.message || 'Gagal mengubah password. Pastikan link tidak kadaluarsa.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    if (!token) {
        return (
            <div className="bg-background-light dark:bg-background-dark font-display text-slate-900 dark:text-white min-h-screen flex flex-col items-center justify-center p-4">
                <div className="text-center bg-white dark:bg-surface-dark p-8 rounded-xl shadow-lg border border-slate-200 dark:border-border-dark max-w-md w-full">
                    <span className="material-symbols-outlined text-red-500 text-5xl mb-4">error</span>
                    <h1 className="text-2xl font-bold mb-2">Link Invalid</h1>
                    <p className="text-slate-500 mb-6">Link reset password tidak valid atau sudah kadaluarsa.</p>
                    <Link to="/forgot-password" className="text-primary hover:underline font-medium">Request Ulang Link</Link>
                </div>
            </div>
        );
    }
        ...
        <div className="bg-background-light dark:bg-background-dark font-display text-slate-900 dark:text-white min-h-screen flex flex-col items-center justify-center p-4 relative">
            ...
            <div className="relative z-10 w-full max-w-[480px] bg-white dark:bg-surface-dark rounded-xl shadow-2xl border border-slate-200 dark:border-border-dark overflow-hidden">
                <div className="px-8 pt-10 pb-2 text-center">
                    ...
                    <h1 className="text-3xl font-bold leading-tight tracking-tight mb-2">Reset Password</h1>
                    <p className="text-slate-500 dark:text-[#9292c8] text-base font-normal">Buat password baru anda.</p>
                </div>
                ...
                <div className="px-8 pb-10 pt-4">
                    <form className="flex flex-col gap-5" onSubmit={handleSubmit}>
                        ...
                        <label className="flex flex-col w-full">
                            <span className="text-sm font-medium leading-normal pb-2 text-slate-900 dark:text-white">New Password</span>
                        ...
                        <label className="flex flex-col w-full">
                            <span className="text-sm font-medium leading-normal pb-2 text-slate-900 dark:text-white">Confirm Password</span>
                            <div className="flex w-full items-stretch rounded-lg group">
                                <div className="relative flex-1">
                                    <span className="absolute left-4 top-1/2 -translate-y-1/2 material-symbols-outlined text-[#9292c8]">lock_reset</span>
                                    <input
